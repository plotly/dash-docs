...
1. **`Procfile`** - Ensure that your project folder has a `Procfile`. See [Application Structure](/dash-enterprise/application-structure) for details. This is often:
 ...
 ```
3. **`requirements.txt`** - Ensure that your project folder has a `requirements.txt` file with the Python dependencies of your app. Don't forgot `gunicorn`! See [Application Structure Documentation](/dash-enterprise/application-structure).
4. **`server = app.server`** - Declare the underlying `server` variable to be referenced by the `web` command in your `Procfile`. See [Application Structure Documentation](/dash-enterprise/application-structure).
5. **`git`** - Install `git` and initialize your project by running `git init`. `git` commits and `git push` is used to deploy your application.
6. **Remove secrets & passwords** - Remove sensitive passwords and API keys from your code and replace them with [Environment Variables](/dash-enterprise/environment-variables).
7. **System-Level dependencies** - If your project requires system level dependencies like database drivers, then install those dependencies with a `predeploy` script and an `apt-packages` file.  See [Application Structure Documentation](/dash-enterprise/application-structure).
8. **Self-contained** - Ensure that all files and datasets that the application code references or saves are within the same project folder. 
9. **Initialize** your app in the Dash Enterprise App Manager UI. See [Deployment Documentation](/dash-enterprise/deployment).
10. **Deploy** your app by running the `git` instructions generated by the App Manager. See [Deployment Documentation](/dash-enterprise/deployment).
...
See the default Python version and how to customize this in the [Application Structure](/dash-enterprise/application-structure) documentation.
